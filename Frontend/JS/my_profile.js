...
// Save user data to local storage and MongoDB
async function save(event) {
    ...
    const profile = {
        ...
        email: localStorage.getItem('email') || "unknown" // Optional: associate with logged-in user
    };
    ...
    try {
        const response = await fetch("http://localhost:3000/save_profile", {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
            ...
            console.log("Saved to MongoDB:", result.message);
        } else {
            console.error("Failed to save profile to MongoDB");
        }
    } catch (error) {
